...
public:
    ...
    void buildHENN(py::object input, py::object ids_ = py::none(), int M = 1, bool best = true)
    {
        ...
        hnswlib::HierarchicalNSW<float> *hnsw = henn::buildHENN(data, num_points, dim, l2space, M, best);
        this->appr_alg = hnsw;
    }

    void addItemsToLayer(py::object input, py::object ids_ = py::none(), int level = -1, int num_threads = -1, bool replace_deleted = false)
    {
        py::array_t<dist_t, py::array::c_style | py::array::forcecast> items(input);
        auto buffer = items.request();
        if (num_threads <= 0)
        ...
        if (level == -1)
        {
            throw std::runtime_error("Level must be specified for HENN");
        }
...

        {
            ...
            if (!ep_added)
            {
                ...
                appr_alg->addPoint((void *)vector_data, (size_t)id, level);
                start = 1;
                ep_added = true;
            }
            ...
            if (normalize == false)
            {
                ParallelFor(start, rows, num_threads, [&](size_t row, size_t threadId)
                    ...
                    appr_alg->addPoint((void*)items.data(row), (size_t)id, level); });
            }
            else
            {
                ...
                ParallelFor(start, rows, num_threads, [&](size_t row, size_t threadId)
                    ...
                    size_t id = ids.size() ? ids.at(row) : (cur_l + row);
                    appr_alg->addPoint((void*)(norm_array.data() + start_idx), (size_t)id, level); });
            }
            cur_l += rows;
        }
    }
    ...
    void addItems(py::object input, py::object ids_ = py::none(), int num_threads = -1, bool replace_deleted = false)
    {
...

        {
            ...
            cur_l += rows;
            std::cout << appr_alg->maxlevel_ << std::endl;
        }
    }
...
PYBIND11_PLUGIN(hnswlib)
{
    ...
    py::class_<Index<float>>(m, "Index")
        ...
        .def("get_hops", &Index<float>::getHops)
